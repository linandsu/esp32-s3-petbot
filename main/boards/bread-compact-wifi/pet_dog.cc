...
{
    action_state_ = state;
    xEventGroupSetBits(action_request_event_, ACTION_REQUEST_EVENT);
}
...
void PetDog::ActionTask()
{
    while (1)
    {
        if (xEventGroupWaitBits(action_request_event_, ACTION_REQUEST_EVENT, pdTRUE, pdTRUE, portMAX_DELAY))
        {
            Action(action_state_);
        }
    }
}
